...
def normalize_qf_findings(qf_result: dict) -> list:
    ...
    for ev in qf_result.get("evaluations", []):
        ...
        if isinstance(evidence_data, list):
            evidence_str = " | ".join(str(e)[:120] for e in evidence_data[:3])
        else:
        ...
        findings.append({
            ...
            "severity": sev,
            "evidence": evidence_str if evidence_str else f"Metric value: {ev.get('metric_value')}",
            "suggested_action": {
                "summary": f"Address {ev.get('id', '')} finding to improve quote feasibility.",
                "priority": sev,
            },
        })
