...
MECHANISMS = {
    "B": {
        ...
        "script": os.path.join(BASE_DIR, "AI Discoverability", "B-Source-Selection",
                               "skills", "audit-orchestrator", "scripts", "orchestrate.py"),
        "function": "run_orchestrated_audit",
    },
    "D": {
        "name": "Entity Resolution",
        "script": os.path.join(BASE_DIR, "AI Discoverability", "D-Entity-Resolution",
...
SUBPROCESS_MECHANISMS = {
    "A": {
        "name": "Crawlability",
        "script": os.path.join(BASE_DIR, "AI Discoverability", "A-Crawlability",
                               "scripts", "crawlability_report.py"),
    },
    "C": {
        "name": "Extractability",
        "script": os.path.join(BASE_DIR, "AI Discoverability", "C-Extractability",
                               "scripts", "extractability_report.py"),
    },
    "F": {
        "name": "Non-Text Lock-In",
        "script": os.path.join(BASE_DIR, "AI Discoverability", "F-Non-Text-Lock-In",
...
def main():
    ...
    for mr in mechanism_results:
        report["mechanisms"][mr["mechanism"]] = {
            "name": mr["name"],
            "status": mr["status"],
            "score": mr.get("result", {}).get("composite_score")
                     or mr.get("result", {}).get("score_ss")
                     or mr.get("result", {}).get("score"),
        }
